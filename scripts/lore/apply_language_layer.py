...
`generate_offspring.py` is known to have written it into:
  ...
  - found and fixed retroactively 2026-08-31 (see fix_placeholder_backstory_text.py and
    fix_encodings_summary_text.py, the one-off cleanup scripts for population debt from before this
    fix existed): 90+ character files across three /generate runs had this exact leftover, because
    the substitution used to be scoped ONLY to knowledge.experience and this entry's own tale/summary,
    never to backstory or to any OTHER tale mentioning the same raw slug as a parent.
