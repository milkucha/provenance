...
only if no existing character slugifies to the same key. A genuine namesake is still fine ("Character C
Gorfalis" alongside "Character C") since the two slugify differently - the check is plain string equality
...
how existing registry/character keys were already derived (e.g. "Character E" -> "character_e"), so this check
doubles as "does _lore/characters/<slug>.json already exist" - no separate index needed.

Usage:
    python scripts/lore/check_character_name.py "Character E"
    python scripts/lore/check_character_name.py "Character C Gorfalis"
"""
