...
- `routines` — a small (2-4), **hand-authored** array of `{location, context, weight,
  ...
  *this* character actually does there). `context` must be a key already present in
  ...
  doesn't describe). `routine_actions` is one line grounding the context in this specific
  character — "blacksmith, values good craft" for a `workshop` routine, not a restatement of the
  context's own generic texture. Weights should sum to roughly 100. This is deliberately never
  auto-generated by `/simulate` itself — same discipline as `backstory` and `criterion`: authored
  here, consumed there.
