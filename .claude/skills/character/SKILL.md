...
## Step 8 — Routines and arc (optional, `/simulate`'s extended mode only)

Only relevant if this character is meant to participate in a `/simulate` run using its extended
materiality mechanic (`.claude/skills/simulate/SKILL.md`'s "Step 3 (extended mode)"). A character
with no `routines` array simply runs in that skill's base mode instead — this step is never
required to finish a character. Author both `routines` and `arc` together, in the same pass — see
`arc` below for why this is no longer split across two different moments/skills.

- `routines` — a small (2-4), **hand-authored** array of `{location, archetype, weight,
  specialization}`. `archetype` must be a key already present in `_lore/archetypes.json` (market,
  workshop, archive, waystation as of this writing — add a new one there by hand if none fits,
  rather than stretching an existing archetype to cover a place-type it doesn't describe).
  `specialization` is one line grounding the archetype in this specific character — "blacksmith,
  values good craft" for a `workshop` routine, not a restatement of the archetype's own generic
  texture. Weights should sum to roughly 100. This is deliberately never auto-generated by
...
- `arc` — **authored at character creation, same discipline as `routines`, not deferred.** Seeded
  from the routine's archetype + personal specialization + criterion, not restated from
  `criterion.anchor` alone: `{about, needs, archetype, premise, resolution: "ongoing", history:
  []}`. `about` is the project's topic tags (at least one `concept: <id>` tag for a genuinely new
  project — run `py scripts/lore/write_arc.py <key> --about ... --needs ... --archetype ... --premise
  "..."` to write it and register the concept in one call, same tool `/simulate` uses). `needs` is
  what it currently requires (checked against other routines' `provides` tags by `/simulate`'s
  `check_needs_provides.py`). Scope the ambition against `scripts/lore/horizon.py`'s band for this
  ...
  On a `transform` (see `/simulate` Step 3), `premise` gets re-composed around whatever `about` the
  transform mechanically injected — same judgment-call moment as re-authoring a failed arc, not a
  separate decision.

  A character reaching `/simulate`'s extended mode with no `arc` on file yet (one that slipped
  ...
  **fallback** for a character who reached extended-mode play without one, not the normal way arcs
  come to exist.

Both fields are entirely `/simulate`'s concern once set — this skill never rolls dice against them
or advances an arc's `history`; it only ever authors the starting values.
