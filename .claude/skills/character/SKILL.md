...
required to finish a character.
...
- `routines` — a small (2-4), **hand-authored** array of `{location, archetype, weight,
  ...
  there.
- `arc` — seeded from the routine's archetype + personal specialization + criterion, not restated
  from `criterion.anchor` alone: `{about, needs, archetype, resolution: "ongoing", history: []}`.
  `about` is the project's topic tags; `needs` is what it currently requires (checked against other
  routines' `provides` tags by `/simulate`'s `check_needs_provides.py`). Scope the ambition against
  `scripts/lore/horizon.py`'s band for this character (never the literal span) — a character
  reading `early` can be given something ambitious; one reading `established`/`late` should get
  something realistically closer to finishable, the same logic Step 6 already uses for how ripe a
  criterion is for change. A character with no `arc` yet gets one authored the first time
  `/simulate`'s extended mode gives them primacy as `home_frame` — this can be left blank here and
  filled in then, rather than forced at character-creation time.
