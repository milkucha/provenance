...
otherwise (README §0 Tier 3 Shipping has the summary). The datapack-side dispatch
(`gesture_<name>.mcfunction`, per-entity timers, the tick loop) is documented in README §0 Tier 2
Handlers "Gesture dispatch", not here.
...
- *To call a gesture* on an NPC mid-dialogue: `execute as @interlocutor run function
  luminacion:npcs/_shared/gesture_<name>` (see "Gesture dispatch" in README §0 Tier 2 Handlers) — tags the NPC
