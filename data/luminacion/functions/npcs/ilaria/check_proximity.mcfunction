# =============================================================================
# Luminacion — Iläria Proximity Check
# =============================================================================
...
#
# Built even though Iläria's movement mode is NONE — this machinery (pause/
# resume tagging + periodic self-heal) is no longer gated on movement mode,
# see _npcs/actions/registry.json -> _action_templates.routine_pause_resume.
# =============================================================================

# A player got within 2 blocks: stop the routine and become interactable.
execute as @e[type=taterzens:npc,name="Iläria",limit=1,tag=!luminacion.paused] at @s if entity @a[distance=..2] run function luminacion:npcs/_shared/pause_routine
...
execute as @e[type=taterzens:npc,name="Iläria",limit=1,tag=luminacion.paused] at @s unless entity @a[distance=..6] run function luminacion:npcs/ilaria/resume_routine
...
scoreboard players add ilaria_heal_cd luminacion.int 1
execute if score ilaria_heal_cd luminacion.int matches 100.. run function luminacion:npcs/ilaria/heal_skin
execute if score ilaria_heal_cd luminacion.int matches 100.. run function luminacion:npcs/ilaria/heal_path
execute if score ilaria_heal_cd luminacion.int matches 100.. run scoreboard players set ilaria_heal_cd luminacion.int 0
